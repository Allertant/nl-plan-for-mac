...
enum PromptTemplates {
    ...
    static func recommendTasks(
        ...
        5. 按候选项类型区别处理：
           ...
           - [项目]：不要推荐整个项目标题，推荐一个今天可执行的小切片，推荐标题必须严格使用"精简项目名: 切片标题"格式（如「搭建博客: 调研食物识别 API」），task_id 设为 null，source_idea_id 填项目 UUID。
           - [项目安排]：不要拆分，推荐标题必须严格使用"精简项目名: 安排内容"格式（如来自「搭建个人博客」的安排「选购域名」，推荐标题为"搭建博客: 选购域名"），task_id 填安排 UUID。
           - 凡是来自项目的推荐项（项目切片 / 项目安排），title 都必须包含且仅包含一个英文冒号 `:`，冒号前必须是精简项目名，冒号后必须是具体行动内容。
           - 不允许返回以下形式：只写切片内容、只写安排内容、直接返回完整项目名、使用中文冒号 `：`、破折号、斜杠或其他分隔符替代 `:`。
          ...
          "recommendations": [
            {
              ...
              "title": "普通想法保持原标题；凡是来自项目则必须严格为\"精简项目名: 具体内容\"",
              "category": "分类",
              "estimated_minutes": 60,
              "reason": "推荐理由"
            }
    ...
    static func generateProjectSlices(
        ...
        6. 推荐标题必须严格使用"精简项目名: 切片内容"格式（如「搭建博客: 调研静态站点生成工具」）
        7. title 必须包含且仅包含一个英文冒号 `:`，冒号前必须是精简项目名，冒号后必须是具体切片行动；不允许只返回切片内容，不允许直接返回项目名，也不允许使用中文冒号 `：` 或其他分隔符替代 `:`
        8. 如果没有合适的时间或项目状态不适合今天推进，返回空列表
