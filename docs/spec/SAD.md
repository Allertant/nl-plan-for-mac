...
| ProjectArrangementRepository | ProjectArrangementEntity 的 CRUD |
...
| 状态 | rawValue | 说明 | 进入时机 |
|------|----------|------|----------|
| pending | `pending` | 待处理 | 初始状态；退回普通想法时（markAttempted=false） |
| inProgress | `in_progress` | 进行中 | 提升为必做项时 |
| attempted | `attempted` | 尝试过 | 结算时关联任务未完成；退回普通想法时（markAttempted=true） |
| completed | `completed` | 已完成 | 结算时关联任务完成 |
| archived | `archived` | 已归档 | `completed` 之后进入的下一阶段，用于归档/隐藏完成项 |

### 3.2 ProjectEntity（项目）
...
|------|----------|------|
| pending | `pending` | 待推进 |
| active | `active` | 进行中（有活跃必做项） |
| archived | `archived` | 已归档 |

### 3.3 ProjectArrangementEntity（项目安排）
...
| deadline | Date? | 截止日期 |
| status | String | 状态（见下表） |
| sortOrder | Int | 排序序号（降序展示，值越大越靠前） |
| createdAt | Date | 创建时间 |
| updatedAt | Date | 更新时间 |

**ArrangementStatus 状态流转：**

| 状态 | rawValue | 说明 | 进入时机 |
|------|----------|------|----------|
| pending | `pending` | 待安排 | 初始状态；退回想法池时（markAttempted=false） |
| inProgress | `in_progress` | 进行中 | 提升为必做项时 |
| attempted | `attempted` | 已尝试 | 结算时关联任务未完成；退回想法池时（markAttempted=true） |
| completed | `done` | 已完成 | 结算时关联任务完成（rawValue 为 done 保持向后兼容） |
| archived | `archived` | 已归档 | `completed` 之后进入的下一阶段，用于归档/隐藏完成安排 |

### 3.4 DailyTaskEntity（必做项）
...
| sourceProjectId | UUID? | 来源项目 ID |
| arrangementId | UUID? | 关联项目安排 ID |
...
| incompletionReason | String? | 未完成原因（结算备注） |
...
|------|----------|------|
| pending | `pending` | 待执行 |
| running | `running` | 执行中（计时中） |
| paused | `paused` | 已暂停 |
| done | `done` | 已完成 |

注意：必做项结算后通过 `isSettled=true` 归档，状态值本身不变。
...
用户点击结束今天
  ...
  → 保存评分到 DailySummaryEntity
    ...
    → 更新关联安排状态：
        任务完成 → completed
        后续如需收纳/隐藏完成安排：completed → archived
        任务未完成 → attempted
    → 更新来源想法状态：
        任务完成 → completed
        后续如需收纳/隐藏完成想法：completed → archived
        任务未完成 → attempted
    → 刷新项目推荐上下文时间
```
...
项目详情页包含两个列表区域，各自的排序规则如下：

**关联必做项**按任务状态分三组，从上到下依次展示：

1. **进行中的任务**（running / paused）— 正在计时或暂停中，按创建时间倒序
2. **已完成但未结算的任务**（done）— 当天标记完成还没到结算时间，按创建时间倒序
3. **已归档的任务**（isSettled=true）— 历史结算过的任务，按结算时间倒序

前两组默认全部展示，第三组默认展示 2 条，可展开加载更多。

**项目安排**按状态分四组，从上到下依次展示：

1. **待安排**（pending）— 新添加的安排，按创建时间倒序（最新在最上）
2. **进行中**（inProgress）— 已提升为必做项的安排
3. **已尝试**（attempted）— 任务未完成退回的安排
4. **已完成 + 已归档**（completed / archived）— 任务完成后的安排，以及后续进入归档阶段的安排

前三组默认全部展示；第四组默认展示条数取决于前几组有多少项（活跃项越多，默认展示的已完成项越少），可展开加载更多。
