...
| 状态 | rawValue | 说明 | 进入时机 |
|------|----------|------|----------|
| pending | `pending` | 待处理 | 初始状态；结算时任务未完成且想法不是 inProgress/completed |
| inProgress | `in_progress` | 进行中 | 提升为必做项时 |
| attempted | `attempted` | 尝试过 | 结算时关联任务未完成 |
| completed | `completed` | 已完成 | （仅历史数据，当前不再自然进入此状态） |
| archived | `archived` | 已归档 | 结算时任务完成且想法为 completed/inProgress |
...
| completed | `done` | 已完成 | （仅历史数据，rawValue 为 done 保持向后兼容） |
| archived | `archived` | 已归档 | 结算时任务完成且安排为 completed/inProgress |
...
用户点击结束今天
  ...
  → archiveAndClearSettledTasks
    ...
    → 更新关联安排状态：
        任务完成 + 安排为 completed/inProgress → archived
        任务未完成 → attempted
    → 更新来源想法状态：
        任务完成 + 想法为 completed/inProgress → archived
        任务未完成 → attempted
    → 刷新项目推荐上下文时间
```
...
4. **已完成 + 已归档**（completed / archived）— 任务完成后归档的安排
